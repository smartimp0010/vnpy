...
class BacktestingEngine(object):
    ...
    def loadHistoryDataCsv(self):
        ...
        df = pd.read_csv(
            'C:/Users/Bette/Desktop/TXF_Tick.csv', index_col=False)

        df = df.rename(
            columns={"Date": "datetime", "Time": "time", "Pid": "symbol", "Dp": "lastPrice", "Dv": "lastVolume",
                     "Bp1": "bidPrice1" \
        ...
        for orderID, order in self.workingLimitOrderDict.items():
            ...
            if buyCross or sellCross:
                ...
                if buyCross:
                    ...
                    # self.pos.append(self.strategy.pos)
    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        if orderType == CTAORDER_BUY:
            ...
            self.OrderBuyTime.append(self.tick.time)
        elif orderType == CTAORDER_SELL:
            order.direction = DIRECTION_SHORT
            order.offset = OFFSET_CLOSE
            self.OrderSell.append(order.price)
            self.OrderSellTime.append(self.tick.time)
    ...
    def calculateBacktestingResult(self):
        ...
        for trade in longTrade:
            result = TradingResult(trade.price, trade.dt, endPrice, self.tick.time,
    ...
    def plotResult(self):
        ...
        if self.plotMode == self.ORDER_MODE:
            ...
            df3 = pd.DataFrame({'Time': self.OrderSellTime, 'SellDeal': self.OrderSell})
            ax3.plot(df1['Time'], df1['S'], color='c', lw=0.8)
            ax3.scatter(df2['Time'], df2['OrderBuy'], color='r')
            ax3.scatter(df3['Time'], df3['SellDeal'], color='g', marker='*')
        else:
            df1 = pd.DataFrame({'Time': self.T, 'S': self.S})
            df2 = pd.DataFrame({'Time': self.BuyDealTime, 'OrderBuy': self.BuyDeal})
            df3 = pd.DataFrame({'Time': self.SellDealTime, 'SellDeal': self.SellDeal})
            ax3.plot(df1['Time'], df1['S'], color='c', lw=0.8)
            ax3.scatter(df2['Time'], df2['BuyDeal'], color='r')
            ax3.scatter(df3['Time'], df3['SellDeal'], color='g', marker='*')
